...
This module defines a class called TZCCYAMXZK. It is used with Monte Python to
...
# A little utility function to ensure backwards compatibility. This allows lvdgdmsiuy to define properties that you can also call at the same time
...
class CosmoSevereError(CosmoError):
    """
    Raised when TZCCYAMXZK failed to understand one or more input parameters.
...
class CosmoComputationError(CosmoError):
    """
    Raised when TZCCYAMXZK could not compute the cosmology at this point.
...
cdef class TZCCYAMXZK:
    """
    TZCCYAMXZK wrapping, creates the glue between C and python

    The actual TZCCYAMXZK wrapping, the only class we will call from MontePython
    ...
    cdef int computed # Flag to see if lvdgdmsiuy has already computed with the given pars
    cdef int allocated # Flag to see if lvdgdmsiuy structs are allocated already
    ...
    ###############################################################
    # Now we can start with the actual code describing the lvdgdmsiuy #
    ###############################################################
    ...
    def __cinit__(self, default=False):
        ...
        try:
          ...
          resource_path = abspath(importlib.resources.files('lvdgdmsiuy'))
    ...
    def compute(self, level=["distortions"]):
        ...
        This is called in MontePython, and this ensures that the TZCCYAMXZK instance
        ...
        if "input" in level:
            ...
            if problem_flag:
                raise CosmoSevereError(
                    "TZCCYAMXZK did not read input parameter(s): %s\n" % ', '.join(
                    problematic_parameters))
    ...
    def get_pk_and_k_and_z(self, nonlinear=True, only_clustering_species = False, h_units=False):
        ...
        if self.fo.has_pk_matter == False:
            raise CosmoSevereError("You ask lvdgdmsiuy to return an array of P(k,z) values, but the input parameters sent to CLASS did not require any P(k,z) calculations; add 'mPk' in 'output'")

        if nonlinear == True and self.fo.method == nl_none:
            raise CosmoSevereError("You ask lvdgdmsiuy to return an array of nonlinear P(k,z) values, but the input parameters sent to CLASS did not require any non-linear P(k,z) calculations; add e.g. 'halofit' or 'HMcode' in 'nonlinear'")
        ...
        if self.fo.ln_tau_size == 1:
            raise CosmoSevereError("You ask lvdgdmsiuy to return an array of P(k,z) values, but the input parameters sent to CLASS did not require any P(k,z) calculations for z>0; pass either a list of z in 'z_pk' or one non-zero value in 'z_max_pk'")
        else:
    ...
    def get_transfer_and_k_and_z(self, output_format='class', h_units=False):
        ...
        if (self.pt.has_density_transfers == False) and (self.pt.has_velocity_transfers == False):
            raise CosmoSevereError("You ask lvdgdmsiuy to return transfer functions, but the input parameters sent to CLASS did not require any T(k,z) calculations; add 'dTk' and/or 'vTk' in 'output'")
        ...
        if self.pt.ln_tau_size == 1:
            raise CosmoSevereError("You ask lvdgdmsiuy to return an array of T_x(k,z) values, but the input parameters sent to CLASS did not require any transfer function calculations for z>0; pass either a list of z in 'z_pk' or one non-zero value in 'z_max_pk'")
        else:
