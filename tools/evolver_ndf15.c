...
/* Absolute floor of the per-variable error weights, wt = max(|y|,abstol).
   Variables smaller than this are only error-controlled in an absolute sense.
   The background integration of covariant scalar-field models (_smg) lowers
   this before calling the evolver and restores it afterwards: on tracker
   solutions the scalar field velocity starts around 1e-43 (phi' ~ a^3 in
   radiation domination from a_ini=1e-14) and would otherwise be integrated as
   unresolved noise, randomly kicking the field off the attractor. Do NOT
   lower it globally: perturbation variables legitimately cross zero, and a
   tiny floor there makes the step-size control demand impossible precision. */
double evolver_ndf15_abstol = 1e-15;
...
int evolver_ndf15(
  ...
  double abstol = evolver_ndf15_abstol, eps=1e-16, threshold=abstol;
  int maxit=4, maxk=5;
